...
def envi_dtype_to_numpy(envi_code):
    ...
    if envi_code not in dtype_map:
        print(f"Uyarı: Bilinmeyen ENVI veri tipi kodu ({envi_code}). float32 varsayılıyor.")
        return np.float32
...
def load_dat(dat_path, metadata):
    ...
    if actual_total != expected_total:
        print(f"UYARI: .hdr boyutları ({lines}×{samples}×{bands} = {expected_total:,}) "
    ...
    # -------------------------------------------------------------------------
    if interleave == "bil":
        print("BIL formatı tespit edildi. (lines, bands, samples) → (lines, samples, bands)")
        data = raw_data.reshape((lines, bands, samples))
        data = data.transpose(0, 2, 1)

    elif interleave == "bsq":
        print("BSQ formatı tespit edildi. (bands, lines, samples) → (lines, samples, bands)")
