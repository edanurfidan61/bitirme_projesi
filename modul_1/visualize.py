...
def make_rgb(data, wavelengths):
    ...
    rgb = np.stack([normalize_channel(red),
                    normalize_channel(green),
                    normalize_channel(blue)], axis=2)
...
def make_leaf_mask(data, wavelengths, method="ndvi"):
    ...
    """
    if HAS_SEGMENTATION:
        print(f"[visualize.py] Maskeleme işlemi 'segmentation.py' modülüne devredilmiş ve '{method}' yöntemi ile uygulanmıştır.")
        return best_mask(data, wavelengths, method=method)
    
    # Fallback (Yedek) Algoritma
    print("[visualize.py] UYARI: 'segmentation.py' dosyasına ulaşılamadığı için temel NDVI maskeleme algoritması uygulanmıştır!")
...
def plot_index_map(index_array, mask, title="İndeks Haritası", cmap="RdYlGn", save_path=None):
    """
...
def plot_spectral_profile(data, mask, wavelengths, title="Ortalama Spektral Profil", save_path=None):
    """
    ...
    ax.plot(wl, mean_spectrum, color="darkblue", linewidth=1.5, label="Ortalama Yansıma")
    ...
    ax.fill_between(wl, mean_spectrum - std_spectrum, mean_spectrum + std_spectrum,
                    alpha=0.3, color="gray", label="± 1 Std Sapma")
...
def plot_mask_overlay(rgb, mask, title="Yaprak Maskesi Overlay", save_path=None):
    ...
    plt.suptitle("Yaprak Maskesinin Doğruluğu Kontrol Edilmiştir", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    if not os.path.exists(HDR_PATH):
        print(f"'{HDR_PATH}' dosyası bulunamamıştır. Modül sahte veriler kullanılarak test edilmektedir...\n")
        ...
        print("Oluşturulan maske RGB üzerine bindirilerek doğruluğu kontrol edilmektedir...")
