...
def calc_all_indices(data, wavelengths):
    ...
    for name, arr in indices.items():
        print(f"  {name:6s} → min: {arr.min():.4f}  max: {arr.max():.4f}  "
              f"mean: {arr.mean():.4f}")
