...
def extract_spectral_means(data, mask, prep_steps=None):
    ...
    if leaf_count == 0:
        bands = data.shape[2]
        print("UYARI: Yaprak maskesinde hiç piksel bulunamamıştır! Sıfır vektörü döndürülmektedir.")
    ...
    if prep_steps and HAS_PREPROCESSING:
        print(f"  Spektral verilere şu ön işleme adımları uygulanmaktadır: {prep_steps}")
    ...
    print(f"  Spektral ortalama: {leaf_count} piksel → ({spectral_means.shape[0]},) vektör")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(HDR_PATH):
        print(f"'{HDR_PATH}' dosyası bulunamamıştır. Sahte veriyle test edilmektedir...\n")
        ...
        if HAS_PREPROCESSING:
            ...
            feat_prep = extract_features(fake_data, fake_wl, fake_mask, prep_steps=["sg", "snv"])
