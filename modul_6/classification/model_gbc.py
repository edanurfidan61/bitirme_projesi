"""
model_gbc.py — Gradient Boosting Sınıflandırma
Konum: modul_6/classification/

Çalıştırma (tekil):
  python model_gbc.py                      # 4-sınıf, y_stress etiketi
  python model_gbc.py --binary             # 2-sınıf, Ph.Eur. 3.5 eşiği (y_flav)
...
OUTPUT_DIR = os.path.join(OUTPUT_DIR, "gbc")
...
def main(binary: bool = False):
    ...
    print("=" * 60)
    print(f"GRADIENT BOOSTING SINIFLANDIRMA  ({'binary Ph.Eur.' if binary else '4-sınıf y_stress'})")
    print("=" * 60)
    ...
    res  = classify_metrics(y_c, y_cv, class_names, label="GBC (CV)")

    suffix = "binary" if binary else "4cls"
    plot_confusion(
        res["confusion_matrix"], class_names,
        title=f"GBC — Confusion Matrix ({suffix})",
        save_path=os.path.join(OUTPUT_DIR, f"confusion_gbc_{suffix}.png"),
    )
    np.savez(
