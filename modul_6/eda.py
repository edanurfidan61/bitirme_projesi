...
def load_data():
    ...
    feat_names = np.load(os.path.join(DATASET_DIR, "feature_names.npy"), allow_pickle=True).tolist()
    csv_path = os.path.join(DATASET_DIR, "dataset_full.csv")
    if os.path.exists(csv_path):
        df_csv = pd.read_csv(csv_path)
        varieties = df_csv["variety"].tolist() if "variety" in df_csv.columns else ["?"] * len(y_chl)
        symptoms = df_csv["symptom"].tolist() if "symptom" in df_csv.columns else ["?"] * len(y_chl)
    else:
        varieties = ["?"] * len(y_chl)
        symptoms = ["?"] * len(y_chl)
...
def plot_target_distributions(y_chl, y_flav, save_dir=None):
    ...
    axes[0].axvline(valid.mean(), color='red', linestyle='--', label=f'Ort={valid.mean():.1f}')
    axes[0].set_xlabel("Klorofil (µg/cm²)"); axes[0].set_ylabel("Yaprak Sayısı")
    axes[0].set_title("Klorofil Dağılımı", fontweight='bold'); axes[0].legend()
    axes[0].text(0.95, 0.95, f"n={len(valid)}\nort={valid.mean():.1f}\nstd={valid.std():.1f}",
    ...
    axes[1].axvline(valid.mean(), color='red', linestyle='--', label=f'Ort={valid.mean():.2f}')
    axes[1].axvline(3.5, color='blue', linestyle=':', linewidth=2, label='Ph.Eur.=3.5')
    axes[1].set_xlabel("Flavonol"); axes[1].set_ylabel("Yaprak Sayısı")
    axes[1].set_title("Flavonol Dağılımı", fontweight='bold'); axes[1].legend()
    axes[1].text(0.05, 0.95, f"n={len(valid)}\nort={valid.mean():.2f}\nstd={valid.std():.2f}",
                 transform=axes[1].transAxes, fontsize=9, va='top',
                 bbox=dict(boxstyle='round', facecolor='lightyellow'))
    plt.tight_layout()
    if save_dir: fig.savefig(os.path.join(save_dir, "target_distributions.png"), dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)


def plot_boxplots(y_chl, y_flav, varieties, symptoms, save_dir=None):
    df = pd.DataFrame({'Chl': y_chl, 'Flav': y_flav, 'variety': varieties, 'symptom': symptoms})
    ...
    for ax, col, by, title in [
        ...
        if 'Flav' in col: ax.axhline(3.5, color='blue', linestyle=':')
    plt.suptitle("Çeşit ve Semptom Bazlı Dağılımlar", fontsize=15, fontweight='bold')
    plt.tight_layout()
    if save_dir: fig.savefig(os.path.join(save_dir, "boxplots.png"), dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)
...
def plot_band_correlation(X, y_chl, y_flav, feat_names, save_dir=None):
    ...
    X_bands = X[:, :n_bands]
    wavelengths = np.array([float(n.replace("band_", "")) for n in feat_names[:n_bands]])
    valid = ~np.isnan(y_chl) & ~np.isnan(y_flav)
    corr_chl = np.array([np.corrcoef(X_bands[valid, i], y_chl[valid])[0, 1] for i in range(n_bands)])
    corr_flav = np.array([np.corrcoef(X_bands[valid, i], y_flav[valid])[0, 1] for i in range(n_bands)])
    ...
    for ax, corr, color, fill, title in [
        ...
        ax.axhline(-0.5, color='red', linestyle=':', alpha=0.5)
        ax.set_ylabel("Pearson r"); ax.set_title(title, fontweight='bold'); ax.grid(True, alpha=0.3)
        top5 = np.argsort(np.abs(corr))[-5:]
        for idx in top5:
            ax.annotate(f"{wavelengths[idx]:.0f}nm\nr={corr[idx]:.2f}", xy=(wavelengths[idx], corr[idx]),
                         fontsize=7, ha='center', bbox=dict(boxstyle='round,pad=0.2', facecolor='yellow', alpha=0.7))
    axes[1].set_xlabel("Dalga Boyu (nm)")
    plt.tight_layout()
    if save_dir: fig.savefig(os.path.join(save_dir, "band_correlation.png"), dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)
...
def plot_index_correlation(X, y_chl, y_flav, feat_names, save_dir=None):
    ...
    all_data = np.column_stack([X_idx[valid], y_chl[valid].reshape(-1,1), y_flav[valid].reshape(-1,1)])
    ...
    ax.set_xticks(range(len(all_names))); ax.set_yticks(range(len(all_names)))
    ...
    plt.tight_layout()
    if save_dir: fig.savefig(os.path.join(save_dir, "index_correlation.png"), dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)
...
def plot_chl_vs_flav(y_chl, y_flav, symptoms, save_dir=None):
    ...
    for sym, c in zip(unique_symp, colors):
        ...
        ax.scatter(y_chl[mask], y_flav[mask], c=[c], label=sym, alpha=0.7, edgecolors='k', linewidth=0.3, s=40)
    ...
    ax.set_xlabel("Klorofil (µg/cm²)"); ax.set_ylabel("Flavonol")
    ax.set_title("Klorofil vs Flavonol", fontweight='bold')
    ax.legend(fontsize=7, ncol=2); ax.grid(True, alpha=0.3); plt.tight_layout()
    if save_dir: fig.savefig(os.path.join(save_dir, "chl_vs_flav.png"), dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)


def main():
    print("=" * 60); print("KEŞİFSEL VERİ ANALİZİ (EDA)"); print("=" * 60)
