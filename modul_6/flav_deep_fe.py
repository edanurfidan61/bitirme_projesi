...
def _bidx(feat, nm):
    best = min(
        [(i, abs(float(f.replace("band_", "")) - nm))
         for i, f in enumerate(feat) if f.startswith("band_")],
        key=lambda x: x[1]
    )
    return best[0]
...
def _range_mean(X_bands, feat, lo, hi):
    idxs = [i for i, f in enumerate(feat)
...
def diagnose_flav(X, feat, y_flav, y_chl):
    ...
    print(f"    CV (std/mean) = {yf.std()/yf.mean():.3f}  "
    ...
    if r_max < 0.5:
        ...
        print("      Bu, hiperspektral data ile flavonol tahmininin genel bir sorunudur")
    ...
    if os.path.exists(csv_path):
        ...
        if "variety" in df.columns:
            ...
            for v, grp in df.groupby("variety")["flav"] if "flav" in df.columns else []:
                pass
            var_stats = df.groupby("variety")["flav"].agg(["mean","std","count"]) \
                if "flav" in df.columns else None
            if var_stats is not None:
                ...
                if within_var > between_var:
                    ...
                    print("      → Stres bilgisi (y_stress) ek özellik olarak eklenmeli!")
...
# ── KAYDET ───────────────────────────────────────────────────────────────────
def save_v3(X_final, feat_final, y_chl, y_flav, y_flav_log, y_stress,
...
def plot_v3(X_final, feat_final, y_flav, y_chl, save_dir=PLOT_DIR):
    ...
    colors_bar = ["#e74c3c" if r_flav[i]>=0.5 else "#3498db" if r_flav[i]>=0.3
