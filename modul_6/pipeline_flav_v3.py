...
def load_data(use_stress=False):
    ...
    X = X[valid]; y_chl = y_chl[valid]
    y_flav = y_flav[valid]; y_stress = y_stress[valid]
    ...
    print(f"  X: {X.shape}, Chl: {np.nanmin(y_chl):.1f}–{np.nanmax(y_chl):.1f}, "
          f"Flav: {np.nanmin(y_flav):.2f}–{np.nanmax(y_flav):.2f}")
    print(f"  PASS (≥{PHEUR_THR}): {(y_flav>=PHEUR_THR).sum()}  "
...
def reg_metrics(y_true, y_pred, label=""):
    ...
    if label:
        print(f"  [{label}]  R²={r2:.4f}  RMSE={rmse:.4f}  RPD={rpd:.2f}  MAPE={mape:.1f}%")
    return dict(R2=r2, RMSE=rmse, RPD=rpd, MAPE=mape)


def pheur_acc(y_true, y_pred):
    return accuracy_score((y_true>=PHEUR_THR).astype(int),
                          (y_pred>=PHEUR_THR).astype(int))
...
def run_decision_tree(X, y_chl, y_flav, feat, tree_plot=False):
    ...
    for target, y, label in [("Chl", y_chl, "Chl"), ("Flav", y_flav, "Flav")]:
        ...
        if tree_plot and target == "Flav":
            ...
            dt_vis = DecisionTreeRegressor(max_depth=4, min_samples_leaf=4,
                                           random_state=42)
            dt_vis.fit(X, y)
            ...
            plot_tree(dt_vis, feature_names=short_feat,
                      filled=True, rounded=True, fontsize=7,
                      max_depth=4, ax=ax)
            ax.set_title("Karar Ağacı — Flavonol (max_depth=4)",
    ...
    param_cls = {"max_depth": [3, 5, 7],
                 "min_samples_leaf": [2, 4, 8],
                 "class_weight": [None, "balanced"]}
    gs_cls = GridSearchCV(
        DecisionTreeClassifier(random_state=42),
        param_cls, cv=cv_s, scoring="accuracy", n_jobs=-1
    )
    gs_cls.fit(X, y_bin)
    ...
    print(classification_report(y_bin, y_pred_cls,
                                 target_names=["FAIL","PASS"], zero_division=0))
    results["cls_acc"] = acc_cls

    if tree_plot:
        dt_cls_vis = DecisionTreeClassifier(max_depth=4, min_samples_leaf=4,
                                             class_weight="balanced", random_state=42)
        dt_cls_vis.fit(X, y_bin)
        short_feat = [f[:20] for f in feat]
        fig, ax = plt.subplots(figsize=(26, 10))
        plot_tree(dt_cls_vis, feature_names=short_feat,
                  class_names=["FAIL","PASS"],
                  filled=True, rounded=True, fontsize=7, max_depth=4, ax=ax)
        ax.set_title("Karar Ağacı Sınıflandırma — Ph.Eur. PASS/FAIL (max_depth=4)",
...
def run_extra_trees(X, y_chl, y_flav, feat):
    ...
    for target, y, label in [("Chl", y_chl, "Chl"), ("Flav", y_flav, "Flav")]:
        ...
        if target == "Flav":
            ...
            ax.barh(range(20), importances[top_idx][::-1], color='darkorange', edgecolor='k')
            ax.set_yticks(range(20))
            ax.set_yticklabels([feat[i] for i in top_idx][::-1], fontsize=9)
            ax.set_xlabel("Önem"); ax.set_title("ExtraTrees — Flav Feature Importance",
                                                  fontweight='bold')
    ...
    gs_cls = GridSearchCV(
        ...
        {"n_estimators":[200,300], "max_depth":[5,8,None], "min_samples_leaf":[1,2,4]},
        cv=cv_s, scoring="accuracy", n_jobs=-1
    )
    gs_cls.fit(X, y_bin)
    best_cls = gs_cls.best_estimator_
    y_pred_cls = cross_val_predict(best_cls, X, y_bin, cv=cv_s)
...
def run_rf_v3(X, y_chl, y_flav, feat):
    ...
    for target, y, label in [("Chl", y_chl, "Chl"), ("Flav", y_flav, "Flav")]:
        ...
        gs = GridSearchCV(
            ...
            {"n_estimators":[200,300], "max_depth":[5,8,None],
             "min_samples_leaf":[1,2,4]},
    ...
    gs_cls = GridSearchCV(
        ...
        {"n_estimators":[200,300], "max_depth":[5,None], "min_samples_leaf":[1,2,4]},
        cv=cv_s, scoring="accuracy", n_jobs=-1
    )
    gs_cls.fit(X, y_bin)
...
def run_svm_v3(X, y_chl, y_flav):
    ...
    for target, y, label in [("Chl", y_chl, "Chl"), ("Flav", y_flav, "Flav")]:
        gs = GridSearchCV(
            pipe,
            {"svr__C":[1,10,100], "svr__gamma":[0.001,0.01],
             "svr__epsilon":[0.05,0.1], "svr__kernel":["rbf"]},
            cv=cv, scoring="r2", n_jobs=-1
        )
        gs.fit(X, y)
    ...
    y_bin = (y_flav >= PHEUR_THR).astype(int)
    cv_s  = StratifiedKFold(n_splits=CV_FOLDS, shuffle=True, random_state=42)
    pipe_cls = Pipeline([("scaler", StandardScaler()),
...
def run_stacking(X, y_chl, y_flav):
    ...
    estimators_cls = [
        ("rf",  RandomForestClassifier(n_estimators=200, max_depth=8,
                                        random_state=42, n_jobs=-1)),
        ("gbr", GradientBoostingClassifier(n_estimators=200, max_depth=3,
                                            learning_rate=0.05, random_state=42)),
...
def print_comparison(all_results, baseline):
    ...
    for name, r in baseline.items():
        if "Chl" in r and "Flav" in r:
            print(f"  {name:<22}  {r['Chl']['R2']:>7.4f}  {r['Chl']['RMSE']:>9.4f}  "
...
def main(use_stress=False, tree_plot=False):
    ...
    baseline = {
        "RF Reg.":  {"Chl": {"R2":0.8270,"RMSE":3.8692,"RPD":2.40},
                     "Flav":{"R2":0.2740,"RMSE":0.4893,"RPD":1.17}},
        "GBR":      {"Chl": {"R2":0.8242,"RMSE":3.9003,"RPD":2.38},
                     "Flav":{"R2":0.2835,"RMSE":0.4862,"RPD":1.18}},
        "SVR":      {"Chl": {"R2":0.8096,"RMSE":4.0584,"RPD":2.29},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stress",    action="store_true",
                        help="y_stress'i özellik olarak ekle")
    parser.add_argument("--tree_plot", action="store_true",
