...
def regression_metrics(y_true, y_pred, label=""):
    ...
    nonzero = y_true != 0
    mape = np.mean(np.abs((y_true[nonzero] - y_pred[nonzero]) / y_true[nonzero])) * 100 if np.sum(nonzero) > 0 else 0.0
    if label:
...
def plot_regression(y_true, y_pred, title="", xlabel="Gerçek", ylabel="Tahmin", save_path=None):
    ...
    ax.legend(loc='lower right'); ax.set_xlim(lims); ax.set_ylim(lims)
    ax.set_aspect('equal'); ax.grid(True, alpha=0.3); plt.tight_layout()
...
def plot_confusion(cm, class_names=None, title="Confusion Matrix", save_path=None):
    import matplotlib.pyplot as plt
    if class_names is None: class_names = [f"Sınıf {i}" for i in range(cm.shape[0])]
    ...
    ax.set_xlabel("Tahmin", fontsize=11); ax.set_ylabel("Gerçek", fontsize=11)
    ax.set_title(title, fontsize=13, fontweight='bold')
    ax.set_xticks(range(len(class_names))); ax.set_yticks(range(len(class_names)))
    ax.set_xticklabels(class_names, fontsize=9, rotation=20, ha='right')
    ax.set_yticklabels(class_names, fontsize=9); plt.tight_layout()
    if save_path: fig.savefig(save_path, dpi=150, bbox_inches='tight')
    plt.show(); plt.close(fig)


def plot_feature_importance(importances, feature_names, top_n=20, title="Feature Importance", save_path=None):
    import matplotlib.pyplot as plt
    ...
    ax.set_yticks(range(top_n)); ax.set_yticklabels(top_names[::-1], fontsize=9)
    ax.set_xlabel("Önem", fontsize=11); ax.set_title(title, fontsize=13, fontweight='bold')
