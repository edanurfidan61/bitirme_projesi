...
try:
    ...
    print(
        "[dataset.py] segmentation.py başarıyla yüklendi (hybrid yöntem kullanılacak)."
    )
except ImportError:
    HAS_SEGMENTATION = False
    print(
        "[dataset.py] UYARI: segmentation.py bulunamadı, visualize.py'deki yedek maskeleme kullanılacak."
    )
...
def build_dataset(
    data_dir, tab_path, output_dir=None, prep_steps=None, seg_method="hybrid"
):
    """
    ...
    for i, (folder_name, hdr_path) in enumerate(leaf_folders):
        ...
        try:
            ...
            feat = extract_features(
                data, meta["wavelengths"], mask, prep_steps=prep_steps
            )

            # Boyut kontrolü
            if feat.shape[0] != n_features:
                print(
                    f"  UYARI: Beklenen {n_features} özellik, "
...
def load_saved_dataset(output_dir):
    ...
    feature_names = np.load(
        os.path.join(output_dir, "feature_names.npy"), allow_pickle=True
    ).tolist()
...
if __name__ == "__main__":
    ...
    if os.path.exists(DATA_DIR) and os.path.exists(TAB_PATH):
        ...
        print(
            f"  Sınıf dağılımı: {dict(zip(*np.unique(y_stress, return_counts=True)))}"
        )
    else:
