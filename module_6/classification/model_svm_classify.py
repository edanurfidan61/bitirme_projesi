"""
model_svm_classify.py — SVM Sınıflandırma
Konum: modul_6/classification/

Çalıştırma (tekil):
  python model_svm_classify.py             # 4-sınıf, y_stress etiketi
  python model_svm_classify.py --binary    # 2-sınıf, Ph.Eur. 3.5 eşiği (y_flav)
...
from utils_model import load_dataset, classify_metrics, plot_confusion
...
OUTPUT_DIR = os.path.join(OUTPUT_DIR, "svm_cls")
...
def main(binary: bool = False):
    ...
    print("=" * 60)
    print(f"SVM SINIFLANDIRMA  ({'binary Ph.Eur.' if binary else '4-sınıf y_stress'})")
    print("=" * 60)
    ...
    res = classify_metrics(y_c, y_cv, class_names, label="SVM (CV)")

    suffix = "binary" if binary else "4cls"
    plot_confusion(
        res["confusion_matrix"],
        class_names,
        title=f"SVM — Confusion Matrix ({suffix})",
        save_path=os.path.join(OUTPUT_DIR, f"confusion_svm_{suffix}.png"),
    )
    np.savez(
