...
def build_features_v2(
    ...
    print(
        f"  [Korelasyon filtresi] Kaldırılan bant: {n_removed_corr}  "
    ...
    if report:
        removed_corr = [
            feat_bands_f[i] for i in range(len(feat_bands_f)) if not corr_mask[i]
        ]
    ...
    poly_features["Deriv520_x_FRI"] = (
        deriv_features.get("Deriv_520_580", np.zeros(X.shape[0])) * lit_features["FRI"]
    )
    ...
    print(
        f"  Varyans/korelasyon ile azaltılan bant : "
...
def save_features_v2(
    X_final, feat_final, y_chl, y_flav, y_flav_log, y_stress, out_dir=OUTPUT_DIR_FE
):
    import shutil
...
def plot_v2_correlation(X_final, feat_final, y_flav, y_chl, save_dir=PLOT_DIR):
    ...
    for ax, r_vals, color, title in [
        ...
        ax.barh(
            list(y_pos), sorted_vals, color=colors_bar, edgecolor="k", linewidth=0.5
        )
        ...
        ax.axvline(
            0.3, color="orange", linestyle=":", alpha=0.7, label="|r|=0.3 (orta)"
        )
        ax.set_xlim(0, 1)
    ...
    plt.suptitle(
        "FE v2 — Yeni Özellikler Korelasyon Analizi\n"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Özellik Mühendisliği v2 (Flavonol Odaklı)"
    )
    parser.add_argument("--plot", action="store_true", help="Korelasyon grafikleri çiz")
    parser.add_argument(
        "--report", action="store_true", help="Konsol korelasyon raporu"
    )
