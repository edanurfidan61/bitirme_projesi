...
def compare(outputs_dir: str = None, use_fe: bool = False, binary: bool = False):
    ...
    for name, folder in reg_models:
        ...
        if os.path.exists(path):
            r = np.load(path)
            cr2 = float(r["chl_r2"])
            crmse = float(r["chl_rmse"])
            crpd = float(r["chl_rpd"])
            fr2 = float(r["flav_r2"])
            frmse = float(r["flav_rmse"])
            frpd = float(r["flav_rpd"])
            print(
                f"  {name:<15s} {cr2:8.4f} {crmse:9.4f} {crpd:8.2f} │ "
    ...
    if reg_data:
        ...
        for ax, r2_vals, title in [
            ...
            colors = [
                "forestgreen" if r > 0.7 else "orange" if r > 0.4 else "red"
                for r in r2_vals
            ]
        ...
        plt.tight_layout()
        fig.savefig(
            os.path.join(compare_dir, "regression_r2_compare.png"),
            dpi=150,
            bbox_inches="tight",
        )
        plt.show()
    ...
    if cls_data:
        ...
        colors = [
            "forestgreen" if a > 70 else "orange" if a > 50 else "red" for a in accs
        ]
        ...
        plt.tight_layout()
        fig.savefig(
            os.path.join(compare_dir, "classification_compare.png"),
            dpi=150,
            bbox_inches="tight",
        )
        plt.show()
